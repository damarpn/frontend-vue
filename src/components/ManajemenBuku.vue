<template>
    <div class="container mt-3">
        <h1 class="text">Manajemen Buku</h1>
        <!-- <p>{{bookList}}</p> -->
        ...
        <div id="formUbah" v-if=" formMode == 'edit' ">
            ...
            <form class="form row" @submit.prevent="submitChange">
                ...
                <div class="col-2">
                    <input type="submit" class="btn btn-primary ml-5" value="Ubah" />
                </div>
            </form>
        </div>
        <div id="daftarBuku">
...
export default {
    props: ["bookList"],
    methods: { // semua event function handling dalam Vue diatur di methods
        submitAdd() {
            // mengambil nilai input form tambah
            const bookCreated = {
            ...
            this.$emit("store", bookCreated); // mengirimkan data 'bookCreated' ke parent (App)
        ...
        showEdit(book, index) {
            this.formMode = "edit";
            this.bookIndex = index; // untuk mendapatkan index (urutan) buku yang diedit
        ...
        submitChange() {
            const bookChanged = this.updatedBook; // mengambil input form dari updatedBook
            const bookIndex = this.bookIndex;
            this.$emit("update", bookChanged, bookIndex); // $emit: mengirimkan sinyal `store` & 2 data ke parent (App)
            this.formMode = ""; // mereset mode form
        },
